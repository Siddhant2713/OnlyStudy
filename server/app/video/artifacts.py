...
SAFE_ARTIFACTS = {"request", "lesson_plan", "screenplay", "storyboard", "storyboard_draft", "validation", "voice_timing", "scene", "screenplay_draft"}
class ArtifactStore:
    ...
    def write_json(self, name: str, value):
        path = self.directory / f"{name}.json"; data = value.model_dump(mode="json") if isinstance(value, BaseModel) else value; path.write_text(json.dumps(data, indent=2), encoding="utf-8"); return path
