...
def persist_job(job: dict) -> None:
    ...
    temporary.write_text(json.dumps({"status": job["status"], "stage": job.get("stage", "planning"), "error": job.get("error")}), encoding="utf-8")
...
def recover_jobs() -> None:
    ...
    for directory in JOBS_DIR.iterdir():
        ...
        try:
            ...
            saved = json.loads(state_path(directory).read_text(encoding="utf-8")) if state_path(directory).exists() else {}
        except (ValueError, OSError, json.JSONDecodeError):
            continue
        final = directory / "final.mp4"
        status = saved.get("status")
        if final.exists():
            status, stage, error = "completed", "completed", None
        elif status in STAGES or status == "queued" or not status:
            status, stage = "failed", saved.get("stage", "planning")
            error = "Generation was interrupted by an API restart. Regenerate the lesson to resume."
        else:
            stage, error = saved.get("stage", "planning"), saved.get("error")
        job = {"request": request, "directory": directory, "status": status, "stage": stage, "error": error}
        if final.exists():
            job["video"] = final
...
def response(job_id):
    ...
    stage = job.get("stage", "planning")
    artifact_paths = {"lesson_plan": "plan", "screenplay": "screenplay", "storyboard": "storyboard", "storyboard_draft": "artifacts/storyboard_draft"}
    return {"id": job_id, "status": job["status"], "stage": stage, "progress": (STAGES.index(stage) + 1) / len(STAGES) if stage in STAGES else (1 if job["status"] == "completed" else 0), "topic": job["request"].topic, "error": job.get("error"), "artifacts": {name: f"/api/lessons/{job_id}/{endpoint}" for name, endpoint in artifact_paths.items() if (job["directory"] / f"{name}.json").exists()}, "video_url": f"/api/lessons/{job_id}/video" if job["status"] == "completed" else None}
def set_stage(job_id, stage):
    with LOCK:
        job = JOBS[job_id]
        job.update(status=stage, stage=stage, error=None)
        persist_job(job)
...
def run_job(job_id, feedback=None, start_stage="planning"):
    ...
    try:
        ...
        with LOCK:
            job.update(status="completed", stage="completed", video=final, error=None)
            persist_job(job)
    except subprocess.TimeoutExpired:
        with LOCK:
            job.update(status="failed", error=f"Manim render exceeded {RENDER_TIMEOUT_SECONDS} seconds")
            persist_job(job)
    except Exception as error:
        with LOCK:
            stage = job.get("stage", "generation")
            job.update(status="failed", error=f"{stage} failed: {error}")
            persist_job(job)
...
def create_lesson(request: LessonRequest, tasks: BackgroundTasks):
    ...
    with LOCK:
        JOBS[job_id] = {"request":request, "directory":directory, "status":"queued", "stage":"planning"}
...
def regenerate(job_id: str, payload: FeedbackRequest, tasks: BackgroundTasks):
    with LOCK:
        ...
        start = route_feedback(payload.feedback); job.update(status="queued", stage=start, error=None)
        persist_job(job)
